...
public class TabDirectory(string path, DataRepoView<NodeView>? dataRepoNodes = null, TabFile.SelectFile? selectFileDelegate = null) : ITab
{
		...
		private List<FileView> GetFiles(Call call)
		{
			try
			{
				return Directory.EnumerateFiles(tab.Path)
					.Select(f => new FileView(f, tab.DataRepoNodes, tab.SelectFileDelegate))
					.ToList();
			}
			catch (Exception ex)
			{
		...
		private List<DirectoryView> GetDirectories(Call call)
		{
			try
			{
				return Directory.EnumerateDirectories(tab.Path)
					.Select(f => new DirectoryView(f, tab.DataRepoNodes, tab.SelectFileDelegate))
					.ToList();
			}
			catch (Exception ex)
			{
...
public class DirectoryView : NodeView, IDirectoryView
{
	...
	public DirectoryView(string path, DataRepoView<NodeView>? dataRepoNodes = null, TabFile.SelectFile? selectFileDelegate = null) :
		base(path, dataRepoNodes)
	{
		Directory = System.IO.Path.GetFileName(path);
		var info = new DirectoryInfo(path);
		LastWriteTime = info.LastWriteTime.Trim();
		Tab = new TabDirectory(path, dataRepoNodes, selectFileDelegate);
	}
}
...
public class FileView : NodeView
{
	...
	public FileView(string path, DataRepoView<NodeView>? dataRepoNodes = null, TabFile.SelectFile? selectFileDelegate = null)
		: base(path, dataRepoNodes)
	{
		...
		if (Filename.EndsWith(".atlas"))
			...
			Tab = new TabFile(path, selectFileDelegate);
	}
}
